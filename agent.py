...
class Agent(base_agent.BaseAgent):
    ...
    def __init__(self, gamma=0.9, learning_rate=0.1, batch_size=64, batch_optim_steps=1, replay_memory_amount=10000,
        ...
        TRAIN: bool variable, when false network does not train
        GAMMA: gamma parameter of update function for Q-values
        LEARNING_RATE: learning rate of optimizer
        BATCH_SIZE: batch size used for optimizing
        BATCH_OPTIM_STEPS: how many times the model is optimized using one batch
        TARGET_UPDATE: when to update the target, e.g. every 10 steps
        STOPPING_CONDITION: threshold between last loss and actual loss when to stop
        EPSILON: epsilon of e-greedy is the inverted probability. Example: 0.9 => 1-0.9 = 0.1 => 10 % probability
         for random action selection 
        LIVE_EPSILON: epsilon that used by the agent
        EXPLORATION_STEPS: how many steps used for exploration / greedy search
    ...
    def setup(self, obs_spec, action_spec):
        ...
        # RMSprop or optional: use adam beta1 0.9 beta2 0.999 epsilon 1e-8
        self.optimizer = torch.optim.RMSprop(self.policy_net.parameters(), lr=self.LEARNING_RATE, eps=0.001,
                                             weight_decay=0.05)

        # read model weights when they exist in the given path
    ...
    def step(self, obs):
        ...
        # every 50k steps reduce random threshold until 10%
    ...
    # concatenates two screen tensors into one tensor
    ...
    # picks action from MOVE_FUNCTION_ID or randomly when not available
